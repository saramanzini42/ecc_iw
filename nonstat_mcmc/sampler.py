...
class JumpProposal(object):
    ...
    def __init__(self, pta, snames=None, empirical_distr=None, f_stat_file=None, save_ext_dists=False, outdir='./chains'):
        ...
        for p in pta.params:
            size = p.size or 1
            # self.pmap[str(p)] = np.arange(ct, ct+size)
            if p.size is not None:
                self.pmap[str(p)] = [list(self.pnames).index(p.name+'_'+str(n)) for n in range(p.size)]
            else:
                self.pmap[str(p)] = [list(self.pnames).index(p.name)]
            ct += size
